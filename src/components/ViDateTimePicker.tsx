...
function splitDateTime(value: string): { date: string; time: string } {
  ...
  return { date, time: raw.length === 5 ? `${raw}:00` : raw };
}

function joinDateTime(date: string, time: string): string {
  if (!date) return "";
  const normalized = !time ? "00:00:00" : time.length === 5 ? `${time}:00` : time.slice(0, 8);
...
function normalizeTimeInput(value: string): string {
  ...
  const seconds = String(Math.min(59, Number(match[3] ?? "0"))).padStart(2, "0");
...
export default function ViDateTimePicker({
  value,
  onChange,
  min,
  max,
...
}: ViDateTimePickerProps) {
  const popupId = useId();
  const rootRef = useRef<HTMLDivElement>(null);
  const [open, setOpen] = useState(false);

  const { date: selectedDate, time: selectedTime } = splitDateTime(value);
  ...
  const selectDate = (isoDate: string) => {
    ...
    onChange(joinDateTime(isoDate, draftTime || selectedTime));
  };

  const commitTime = (rawTime: string) => {
    const time = normalizeTimeInput(rawTime);
    setDraftTime(time);
    if (selectedDate) {
      onChange(joinDateTime(selectedDate, time));
    }
  };
    ...
    <div ref={rootRef} className={cn("relative min-w-0", className)}>
      ...
      >
        <span className={cn("truncate text-left", value ? "text-gray-900" : "text-gray-400")}>
          {value ? formatViDateTime(value) : placeholder}
        </span>
      ...
      {open ? (
        ...
        >
          ...
          <div className="mt-3 border-t border-gray-100 pt-3">
            <label className="mb-1.5 block text-xs font-medium text-gray-500" htmlFor={`${id ?? "dt"}-time`}>
              Giờ (HH:mm:ss)
            </label>
            <input
              id={`${id ?? "dt"}-time`}
              ...
              className={cn(
                FILTER_CONTROL_HEIGHT_CLASS,
                INPUT_FIELD_BORDER_CLASS,
                "w-full px-3 text-sm tabular-nums text-gray-900"
              )}
              value={draftTime}
